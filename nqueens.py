...
class NQueenBoard:
	...
	def setNumberQueens(self):
		"""Get number of queens for puzzle and assign to numQueens variable."""
		
		print("Enter the number of Queens to be used in this puzzle >\n")
		aNumber = 0
		while True:
			aNumber = int(input())
			if aNumber > 0:
	...
	def generateBoard(self):
		...
		for x in range(self.numQueens):
			...
			tempList = self.generateEmptyRow()
...
def main():
    ...
    if(arg_length > 1):
        ...
        aBoard = NQueenBoard()
        aBoard.setNumberQueens()
            
        aBoard.setInitialBoard()
        aBoard.setRandomTarget()
        aBoard.generateHeuristicBoard()

def initializeBoardObject(aBoard):
	"""NOT USED YET. PLACEHOLDER FOR NEEDED FUNCTIONS."""        
	aBoard.setNumberQueens()
